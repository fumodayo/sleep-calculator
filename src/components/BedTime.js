...
const BedTime = () => {
  ...
  const realTime = () => {
    const time = new Date(context.value);
    const seconds = time.getHours() * 3600 + time.getMinutes() * 60;
    return new Date(seconds * 1000).toISOString().substring(11, 16);
  };

  const ListBedTime = context.handleCalculateBedTime(context.value);
    ...
    <Stack spacing="20px" justifyContent="center" alignItems="center">
      ...
      <Stack>
        ...
        <Typography
          ...
          style={{ display: "inline-block" }}
        >
          To wake up refreshed at {realTime()}, you need go to sleep at one of the
          following times:
        </Typography>
      </Stack>
      <Grid
